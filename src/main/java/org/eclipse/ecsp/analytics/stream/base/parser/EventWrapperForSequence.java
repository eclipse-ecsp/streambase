...
/**
 * class EventWrapperForSequence implements EventWrapperBase.
 */
public class EventWrapperForSequence implements EventWrapperBase {
    ...
    /** The logger. */
    private static IgniteLogger logger = IgniteLoggerFactory.getLogger(EventWrapperForSequence.class);
    ...
    /**
     * Instantiates a new event wrapper for sequence.
     *
     * @param events the events
     */
    public EventWrapperForSequence(List<?> events) {
     ...
     * @param e the e
     */
    public EventWrapperForSequence(byte[] rawEvent, Exception e) {
    ...
    /**
     * Can qualify the results of navigating a path with additional operators.
     * For ex data[EventID=EngineRPM].Data.value will look up the
        ...
        for (int i = 0; i < splits.length; i++) {
            if (ret instanceof List) {
                ...
                if (ret instanceof List<?> list && qlist.isPresent()) {
                    List<?> retList = list;
                    if (!retList.isEmpty()) {
                        ret = getRetObject(splits, ret, i, qlist, retList);
                    }
                }
            } else if (ret instanceof Map<?, ?> map) {
     ...
     *
     * @param name the name
     * @param byFieldToBeSorted         : Ex : TimeStamp, Version
     ...
     */
    private Object getRetObject(String[] splits, Object ret, int i, Optional<Qualifier[]> qlist, List<?> retList) {
        if (retList.get(0) instanceof Map && qlist.isPresent()) {
            List<Map<?, ?>> flist = filterList((List) ret, qlist.get());
    ...
    public Object getProperty(Map wrapper, String name) {
        ...
        if (splits.length == Constants.TWO) {
            ...
            if (r instanceof Map<?, ?> map) {
                return map.get(splits[1]);
            } else if (r instanceof List<?> list) {
                return ((Map) list.get(0)).get(splits[1]);
            } else {
                return null;
            }
        }
    ...
    /**
     * Return one event after sorting by sorting criteria. Ex : Say we have
     * multiple EID=Location events in the message, and we want the Location
     * event which has the latest TimeStamp.

     * @param retAsList List
     ...
     *            
     * @return A map containing entries sorted by a field
     */
