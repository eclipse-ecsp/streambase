...
@TestPropertySource("/hivemq-test-ssl-mqtt.properties")
public class HiveMQMqttDispatcherIntegrationSSLTest {

    /** The logger. */
    private static IgniteLogger logger = IgniteLoggerFactory.getLogger(HiveMQMqttDispatcherIntegrationSSLTest.class);
    ...
    public void setup() {
        ...
        subscribeMqttClient = MqttClient.builder().identifier(UUID.randomUUID().toString())
                .serverHost("localhost").serverPort(TestConstants.PORT)
    ...
    public void testClientConnection_without_topic_pefix() throws InterruptedException {
        hiveMqMqttDispatcher.isHealthy(true);
        ...
        String mqttTopicToSubscribe = defaultMqttTopicNameGeneratorImpl
                .getMqttTopicName(key, value.getDeviceMessageHeader(), null).get();
