...
public class DLQRetryHandlerFailureTest extends KafkaStreamsApplicationTestBase {

    /** The Constant LOGGER. */
    private static final Logger LOGGER = LoggerFactory.getLogger(DLQRetryHandlerFailureTest.class);
    ...
    /**
     * Tests the DLQ re-processing flow logic for faliure with dlq reprocessing
     * performed max retry attempt.
     *
     * @throws Exception exception
     */

    @Test
    public void testDLQReprocessingForFailure() throws Exception {
        ...
        Thread.sleep(TestConstants.THREAD_SLEEP_TIME_60000);
        KafkaTestUtils.getMessages("test-dlq", consumerProps, 1, TestConstants.TEN_THOUSAND);
        try {
            Assert.assertEquals(1, DLQReprocessingPreProcessorOne.count);
            Assert.assertEquals(1, DLQReprocessingPreProcessorTwo.count);
            Assert.assertEquals(0, DLQReprocessingPostProcessorOne.count);
            Assert.assertEquals(0, DLQReprocessingPostProcessorTwo.count);
        } catch (Exception e) {
            e.printStackTrace();
        }
        shutDown();
    ...
    public static final class DLQReprocessingMaxRetryServiceProcessor
        ...
        public void process(Record<IgniteKey<?>, IgniteEvent> kafkaRecord) {
            ...
            LOGGER.info("Test DLQ Reprocessing ---> Key {} Value {} event id {}", kafkaRecord.key(), 
                    value, value.getEventId());
