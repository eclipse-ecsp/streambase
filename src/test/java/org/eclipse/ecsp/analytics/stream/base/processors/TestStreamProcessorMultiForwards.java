...
/**
 * class {@link TestStreamProcessorMultiForwards} implements {@link IgniteEventStreamProcessor}.
 */
@Component
public class TestStreamProcessorMultiForwards implements IgniteEventStreamProcessor {
    ...
    public String[] sources() {
        if (log.isDebugEnabled()) {
            log.debug("source topics {}", Arrays.toString(sourceTopics));
        }
        return sourceTopics;
    }
    ...
    public String[] sinks() {
        if (log.isDebugEnabled()) {
            log.debug("sink topics {}", Arrays.toString(sinkTopics));
        }
        return sinkTopics;
    }
    ...
    public void process(Record<IgniteKey<?>, IgniteEvent> kafkaRecord) {
        ...
        if (null == key) {
            log.error("Key is null, no further processing.");
        }

        if (null == value) {
            log.error("Value is null, no further processing.");
        }
        ...
        log.info("Sending key value pair first time");
        this.ctxt.forward(kafkaRecord, sinkTopics[0]);

        log.info("Sending key value pair second  time");
        this.ctxt.forward(kafkaRecord, sinkTopics[0]);

        log.info("Sending key value pair third  time");
        this.ctxt.forward(kafkaRecord, sinkTopics[0]);

        log.info("Sending key value pair to a different topic ");
        this.ctxt.forward(kafkaRecord, sinkTopics[1]);
    ...
    public void initConfig(Properties props) {
        ...
        log.info("sink topic list {}", sinkTopicNames);
